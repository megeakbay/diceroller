...
def retry_step(
    ...
    parts = [
        ...
        png_part(after),
        text_part(build_step_prompt(step_meta) + RETRY_NOTE.format(reason=reason)),
    ]

    try:
        parsed = generate_json(
            client, model, parts,
            required_fields=["reasoning", "top", "bottom"],
    ...
    return make_record(step_meta, parsed)
...
def main() -> None:
    ...
    for i, puzzle_dir in enumerate(puzzle_dirs, 1):
        ...
        if local_retried:
            ...
            print(
                f"[{i}/{len(puzzle_dirs)}] {puzzle_dir}: "
                f"retried {local_retried}, fixed {local_fixed}"
            )
